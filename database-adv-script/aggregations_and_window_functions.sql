...
SELECT property_id, name, total_bookings, RANK() OVER (ORDER BY total_bookings DESC) AS property_rank
FROM (
...
-- 3. Use ROW_NUMBER() to assign sequential row numbers to properties based on booking count
SELECT 
    property_id, 
    name, 
    total_bookings, 
